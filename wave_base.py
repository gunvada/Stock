...
def classify_base(highs, lows, closes, min_bars=MIN_BARS, base_window=BASE_WINDOW):
    """다년 주봉 고/저/종가 리스트(오래된→최신)로 베이스 위치를 근사 분류.
    min_bars/base_window 를 바꾸면 일봉·분봉 등 다른 타임프레임에도 적용 가능
    (단, 캔들이론 원전은 1시간 미만 차트의 신호를 '대부분 무의미'로 본다).
