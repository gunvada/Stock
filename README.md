...
## 📉 백테스트 검증 결과 (283거래일 / 6,400+ 후보 / 2,000+ 종목)

`analyze_*.py` 로 2025-05~2026-06 대표본을 검증한 **정직한 결론**: 이 폭증주들에
대한 **기계적 진입/청산 규칙에는 검증된 양(+)의 엣지가 없다.**

| 전략 | 순익/거래 | 판정 |
|---|---|---|
| 다음날 시초→종가 보유 | −2.9% | 엣지 없음 |
| 프리마켓 06:30→09:30 개장가 청산 | −1.1% | 엣지 없음 |
| 프리마켓 +8% TP | −0.2% | break-even |
| 프리마켓 TP10 / STOP10 | −0.6% | 엣지 없음 |
| 캔들신호·종합점수 변별력 | ~0 | 모든 변형서 무력 |

- 종목들은 장중 **+9~14% 솟구치나(펌프)** 그 타이밍이 규칙으로 안 잡히고,
  왕복 **2.5% 비용**이 저가주에 큰 허들. 작은 표본에서 보였던 "top6 +3%",
  "캔들 강한매수 우위", "직전 100%+ 모멘텀"은 모두 **대표본에서 소멸(노이즈)**.
- 단, **직전 7일 +200% 극단 급등**은 다음날 −6.1%로 뚜렷이 불리(과열 상한 근거).

> **∴ 이 도구의 정직한 용도 = 재량적 워치리스트.** 장 전에 '주목할 종목'을
> 좁혀주는 발굴/모니터링 도구이며, 픽은 본인의 눈·뉴스·호가로 직접 판단할
> **후보**다. 누적 장부(`*_ledger.csv`)는 자동 수익이 아니라 학습·기록용.
